...
class FractalControl {
  constructor(baseLineData, generatorData) {
    ...
    this.updateGeneratorValuesFromPoints = () => {
      ...
      const genPoints = this.points.map((point) => transformPointReverse(point, this.baseLine)
      );
    ...
    this.onMove = (eventPoint, delta) => {
      ...
      for (const [lineIndex, line] of this.lines.entries()) {
        if (line.isTriangleDragging) {
          ...
          if (actions.swap) {
            line.swapPoints();
            [genLine.start, genLine.end] = [genLine.end, genLine.start][(mirrorLine.start, mirrorLine.end)] = [mirrorLine.end, mirrorLine.start];
          }
